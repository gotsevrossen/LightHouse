...
# Protect the credential banner, database, configuration and raw event payloads.
& icacls.exe $DataDir /inheritance:r /grant:r '*S-1-5-18:(OI)(CI)F' '*S-1-5-32-544:(OI)(CI)F' | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'Unable to protect LightHouse data directory.' }
...
function Fetch([string]$Url, [string]$Name) {
    ...
    Write-Host "$Name SHA256 $((Get-FileHash $target -Algorithm SHA256).Hash)"
    return $target
}
...
try {
    ...
    if (!(Get-Service npcap -ErrorAction SilentlyContinue)) {
        if ($config.NpcapOemInstaller) {
            Run $config.NpcapOemInstaller '/S /winpcap_mode=yes'
        } elseif ($Unattended) {
    ...
    if (!(Test-Path "$AppDir\tools\nssm.exe")) {
        ...
        $nssm = Get-ChildItem "$DataDir\cache\nssm" -Filter nssm.exe -Recurse | Where-Object { $_.Directory.Name -eq 'win64' } | Select-Object -First 1
        Copy-Item $nssm.FullName "$AppDir\tools\nssm.exe"
    }
    ...
    'Installation completed.' | Set-Content "$AppDir\setup\last-result.txt" -Encoding utf8
    Stop-Transcript | Out-Null
    exit 0
} catch {
    Write-Host "INSTALLATION FAILED: $_"
    "Installation incomplete: $_" | Set-Content "$AppDir\setup\last-result.txt" -Encoding utf8
    Stop-Transcript | Out-Null
    exit 1
}
